 ...
 */
 ...
 const bankBox = document.getElementById("div_itemBank")
 ...
 function dragstartevent(event) {
  ...
  if(draggable.parentNode.classList.contains("itemStack")) { //Checks if the item is coming from the bank
    /*--*/
    if (numItems[draggable.id] > 0) /*Checks if a new item can be created*/
    {
 ...
 function dragoverevent(container) {
  ...
  {
    update_counters(draggingCurrent.id) //Updates counters with an extra value added to the stack to show what will happen if you drop an item there
    container.appendChild(draggingCurrent)
 ...
 function update_counters(plusOne)
 {
   ...
   allItems.forEach(itm => {
     ...
     else { //Only adds empty stack class to things that are in the bank
      if (numItems[itm.id] == 0)
      {
